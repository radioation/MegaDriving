...
*------------------------------------------------
    ...
    .set PAL0_COLOR4, 0xC0080000
		...
		.set LINE_DARK_COLOR, 0x4AA
		.set LINE_LIGHT_COLOR, 0xAFF

		.set GRASS_DARK_COLOR, 0x084
		.set GRASS_LIGHT_COLOR, 0x0B0
...
WORK:
	...
	lea colors, %a0         /* get 'colors' array effective address to A0 */
 	move.w  (%a0,%d0.w),%d0	/* check shading for current scanline */
	move.l	(%sp)+, %a0     /* restore address register 0 */

	tst.w	%d0               /* check shading value */
...
# 0 unlit, 1 light, 2 dark
	.comm	lineColor,2,2 
	.comm	grassColor,2,2


	.comm	horizonLine,2,2

	.even
	.globl	VscrollA
	.section	.bss
	.align	2
	.type	VscrollA, @object
	.size	VscrollA, 448
VscrollA:
	.zero	448
